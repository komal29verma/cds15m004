...
GPIO_PORTB_DATA_R = 0x01; 
		SysLoad(800000);
    RSW = GPIO_PORTD_DATA_R&0x01;     // read PD0 into SW1
    LSW = GPIO_PORTD_DATA_R&0x02;     // read PD1 into SW2
	if(!RSW && !LSW){    //both not pressed
GPIO_PORTA_DATA_R = 0x01;
GPIO_PORTB_DATA_R = 0x01;    
} else
if(!RSW && LSW){    //left sw pressed
GPIO_PORTE_DATA_R = 0x01;  //right forward
GPIO_PORTB_DATA_R = 0x02;   //left backward 
} else
if(RSW && !LSW){  //right sw pressed
GPIO_PORTE_DATA_R = 0x02;  // right backward
...
void PortE_Init(void)
{
	...
	GPIO_PORTE_DIR_R |= 0x03; // PA0.PA1 output
	GPIO_PORTE_AFSEL_R &= ~0x03; // not alternative
	GPIO_PORTE_AMSEL_R &= ~0x03; // no analog
	GPIO_PORTE_PCTL_R &= ~0x000000FF; // bits for PA1,PA0
	GPIO_PORTE_DEN_R |= 0x03; // enable PA0,PA1
}
